...
import java.util.*;
class GoalStackPlanning{
	static Stack<String> goalStack = new Stack<>();
	static String start_state="ONTABLE(A) ^ ONTABLE(C) ^ ON(B,A) ^ ONTABLE(D) ^ ARMEMPTY ^ CLEAR(D) ^ CLEAR(C) ^ CLEAR(B)";
	static String goal_state="ONTABLE(A) ^ ONTABLE(B) ^ ON(C,B) ^ ON(D,C) ^ ARMEMPTY ^ CLEAR(D) ^ CLEAR(A)";
